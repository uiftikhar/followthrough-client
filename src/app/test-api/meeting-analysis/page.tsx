...
export default function TestMeetingAnalysisPage() {
  ...
  const startTest = async () => {
    ...
    try {
      ...
      const sessionResponse = await fetch(
        `${apiBaseUrl}/api/v1/analysis/sessions`,
        {
          method: "POST",
      ...
      const analyzeResponse = await fetch(
        `${apiBaseUrl}/api/analysis/sessions/${newSessionId}/analyze`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!analyzeResponse.ok) {
        throw new Error(
          `Failed to submit transcript: ${analyzeResponse.status}`,
        );
      }
    ...
    const pollInterval = setInterval(async () => {
      ...
      try {
        const resultsResponse = await fetch(
          `${apiBaseUrl}/api/analysis/sessions/${sid}/results`,
        );
        ...
        addLog(
          `Poll ${pollCount}: Status = ${currentStatus}, Progress = ${currentProgress}%`,
        );
        ...
        if (currentStatus === "completed") {
          ...
          setStatus("completed");
          setResults(
            resultsData.data.results?.results || resultsData.data.results,
          );
  ...
  const deleteSession = async () => {
    ...
    try {
      ...
      const deleteResponse = await fetch(
        `${apiBaseUrl}/api/analysis/sessions/${sessionId}`,
        {
          method: "DELETE",
        },
      );
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="mb-6 text-3xl font-bold">Meeting Analysis API Test</h1>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent>
            <div className="h-[300px] overflow-auto rounded bg-black p-4 font-mono text-sm text-green-400">
              {logs.length === 0 ? (
                <p className="text-gray-500">
                  Logs will appear here when you start the test
                </p>
              ) : (
