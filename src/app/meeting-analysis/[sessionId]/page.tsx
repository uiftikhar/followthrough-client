...
export default async function MeetingAnalysisPage({ params }: PageProps) {
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="mb-6 text-2xl font-bold">Meeting Analysis Results</h1>
      ...
      <ErrorBoundary
        fallback={
          <Alert variant="destructive">
            ...
            <AlertDescription>
              There was a problem loading the analysis results. This may be due
...
async function AnalysisResults({ sessionId }: { sessionId: string }) {
  try {
    ...
    return (
      <ResultVisualizationWrapper initialData={results} sessionId={sessionId} />
    );
  } catch (error) {
