    ...
    try {
      ...
      setMessages((prev) => [
        ...prev.filter((msg) => (msg as ChatResponse).type !== "loading"), // Remove loading message
        {
          id: `error-${Date.now()}`,
          content:
            "Sorry, there was an error uploading your transcript. Please try again.",
          type: "error",
          timestamp: Date.now(),
        },
      ]);
        ...
        setMessages((prev) => {
          ...
          const analysisMessageIndex = updatedMessages.findIndex(
            (msg) =>
              ...
              (msg as ChatResponse).attachments?.some(
                (att) => att.data?.meetingId === meetingId,
              ),
          );
        ...
        if (statusResponse.status === "completed") {
          ...
          setMessages((prev) => [
            ...prev,
            {
              ...
              content:
                "Analysis complete! You can now ask questions about the meeting.",
              type: "text",
              timestamp: Date.now(),
            },
          ]);
          ...
          setMessages((prev) => [
            ...prev,
            {
              id: `failed-${Date.now()}`,
              content:
                "Analysis failed. Please try again or upload a different transcript.",
              type: "error",
              timestamp: Date.now(),
            },
          ]);
