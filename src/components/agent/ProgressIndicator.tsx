...
export default function ProgressIndicator({
  ...
  const { progress, status, isLoading, error, details } = useAgentProgress(
    sessionId,
    {
      pollingInterval,
  ...
  if (error) {
    return (
      <div
        className={`rounded border border-red-200 bg-red-50 p-3 ${className}`}
      >
      ...
      {showDetails && details && (
        <div className="mt-2 text-xs text-gray-500">
          ...
          {status === "error" && details.error && (
            <p className="mt-1 text-red-500">Error: {details.error}</p>
          )}
        </div>
      )}
    </div>
